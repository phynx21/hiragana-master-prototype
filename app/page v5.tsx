...
// --- DATA MAPPING ---
const characters = [
  { char: 'あ', story: 'The hero is falling off a cliff... "Aaaaah!"', image: '/hero-cliff.png', hasVideo: true },
  { char: 'い', story: 'Ouch! He stubbed his toe... "Itai!"', image: '/hurt-leg.png', hasVideo: false },
  { char: 'う', story: 'Success! He found the map... "Ureshii!"', image: '/happy-hero.png', hasVideo: false },
  { char: 'え', story: 'Keep going with a smile... "Egao!"', image: '/hero-smile.jpg', hasVideo: false },
  { char: 'お', story: 'Delicious! He finally found food... "Oishii!"', image: '/hero-eating.gif', hasVideo: false },
];
...
export default function HiraganaV3() {
  ...
  const handleStart = () => {
    ...
    // Only attempt to play if the first character has a video
    if (current.hasVideo) {
      setTimeout(() => {
            ...
            {phase === 'STORY' && (
              <motion.div key="story" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col items-center">
                ...
                <div className="relative w-full aspect-square rounded-[2rem] bg-slate-100 overflow-hidden mb-8 shadow-inner border-2 border-slate-50">
                  ...
                  {/* CONDITIONAL VIDEO: Only renders if hasVideo is true */}
                  {current.hasVideo && (
                    <motion.div
                ...
                <button 
                  onClick={(e) => {
                    ...
                    if (step < characters.length - 1) {
                      ...
                      alert("You've mastered the first set!");
                      setStep(0);
                      setPhase('STORY');
                    }
                  }}
